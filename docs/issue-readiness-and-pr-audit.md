...
6. If they are aligned, complete the design-admission check required by [`AGENTS.md`](../AGENTS.md) when the proposed design introduces a material persistent mechanism. Do not treat a small diff, few files, or few lines as proof that the mechanism is simple or necessary.
