...
An issue can move back from `ready-for-implementation` when new contract information, architecture drift, current-code reality, or a scope change reopens a design, product, or dependency question. `ready-for-implementation` is not a permanent assertion that the Issue still matches the repository when implementation begins. It settles the contract, not the admission of a new persistent mechanism: when implementation would introduce material persistent complexity, the Engineer must complete the author-side design-admission check and obtain the independent, non-author design-ablation pass required by [`AGENTS.md`](../AGENTS.md) before coding. Do not hide an unresolved decision inside an apparently ready issue; name the decision and route it to its owner first.
...
6. If they are aligned, complete the author-side design-admission check and obtain the independent, non-author design-ablation pass required by [`AGENTS.md`](../AGENTS.md) when the proposed design introduces a material persistent mechanism. Do not treat a small diff, few files, or few lines as proof that the mechanism is simple or necessary.
7. If the design is admitted, load specialized policy or skills for the actual risk surface and make the smallest complete coherent change that satisfies the issue and current architecture.
