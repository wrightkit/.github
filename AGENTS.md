...
7. If the proposed design introduces a material persistent mechanism, complete the author-side design-admission check before implementation and obtain a separate design-ablation pass from an independent agent or reviewer who did not author the design: identify each new abstraction, dependency, state/lifecycle mechanism, API/protocol, committed script or tool, behavior-driving metadata/schema layer, configuration surface or feature switch, fallback/compatibility path, or cross-repository contract; establish the current gap that requires it; and attempt removal, deferral, inlining, reuse, or a simpler direct alternative. Keep only mechanisms whose absence breaks a concrete requirement, invariant, ownership boundary, acceptance criterion, or known workflow, or creates a higher total maintenance obligation. Trivial or local changes remain lightweight and do not require a separate design ceremony.
8. If they are aligned and the design survives that admission check, load the specialized policy or skills indicated by the actual risk surface and implement the smallest complete coherent change.
...
For substantive designs, an independent agent or reviewer who did not author the design must run a design-ablation pass before implementation using the design-admission rule above. The author's admission check does not substitute for that independent pass. After implementation, do not declare the change complete until an independent agent or reviewer who did not author the implementation has run a separate implementation-ablation pass. The pre-implementation check does not prove that the implemented behavior satisfies the contract.
