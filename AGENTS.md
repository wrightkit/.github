...
7. If the proposed design introduces a material persistent mechanism, perform design admission before implementation: identify each new abstraction, dependency, state/lifecycle mechanism, API/protocol, committed script or tool, behavior-driving metadata/schema layer, configuration surface or feature switch, fallback/compatibility path, or cross-repository contract; establish the current gap that requires it; and independently attempt removal, deferral, inlining, reuse, or a simpler direct alternative. Keep only mechanisms whose absence breaks a concrete requirement, invariant, ownership boundary, acceptance criterion, or known workflow, or creates a higher total maintenance obligation. Trivial or local changes remain lightweight and do not require a separate design ceremony.
...
For substantive designs, perform an independent design-ablation pass before implementation using the design-admission rule above. After implementation, do not declare the change complete until an independent agent/reviewer who did not author it has run a separate implementation-ablation pass. The pre-implementation check does not prove that the implemented behavior satisfies the contract.
